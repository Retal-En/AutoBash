...
class DiscountMatrix(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company',
                                 default=lambda self: self.env['res.company']._company_default_get('discount.matrix'))
    discount_account_id = fields.Many2one("account.account", string="Discount Account",
                                          domain=[('deprecated', '=', False)], help="Account used for Discount")
    income_account_id = fields.Many2one("account.account", string="Income Account", domain=[('deprecated', '=', False)],
                                        help="Account used for Discount")
    payment_type = fields.Selection([('cash', 'Cash'), ('installment', 'Installment'), ('crf', 'CFR')], 'Payment Type',
...
class DiscountMatrixLine(models.Model):
    ...
    commission_percentage = fields.Float(string="Rep's Due Commission Percentage (%)", track_visibility='onchange',
                                         required=True, )
    matrix_id = fields.Many2one('discount.matrix', 'Discount Matrix')
    payment_method = fields.Selection([('cash', 'Cash'), ('installment', 'Installment'), ('crf', 'CFR')],
                                      'Payment Method', default='cash')

    company_id = fields.Many2one('res.company', 'Company',
                                 default=lambda self: self.env['res.company']._company_default_get(
                                     'discount.matrix.line'))
...
class DealCommission(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company',
                                 default=lambda self: self.env['res.company']._company_default_get('deal.commission'))
...
class ServiceConfig(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company',
                                 default=lambda self: self.env['res.company']._company_default_get('service.config'))
...
class FinanceConfig(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company',
                                 default=lambda self: self.env['res.company']._company_default_get('finance.config'))
